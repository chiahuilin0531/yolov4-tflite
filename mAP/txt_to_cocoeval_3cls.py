...
parser.add_argument('--label_dir', '--gt', default=None,type=str, help="label_dir which contains txts or label_json in coco format")
parser.add_argument('--pred_dir', '--dt', default=None,type=str, help="prediction_dir which contains txts or prediction_json in coco format")
...
def yolo2coco(label_path, pred_path):
    ...
    if '.json' == label_path[-5:]:
        ...
        classes = [{"supercategory": "all","id": 0,"name": "Green"},{"supercategory": "all","id": 1,"name": "Red"},{"supercategory": "all","id": 2,"name": "Yellow"}]
        i = 0
        ...
        for file in tqdm(files):
            ...
            for line in f1.readlines():
                ...
                h = float(y2) - y1
                bb = [x1, y1, w, h]
                cls_name=convert_clsid(c)
                labels.append({'id':i, 'image_id':int(file[:-4]), 'category_id':cls_name, 'bbox':[round(b, 3) for b in bb], 'area':bb[2]*bb[3], 'iscrowd':0})
                i += 1
            f1.close()
    ...
    if '.json' == pred_path[-5:]:
        ...
        for file in tqdm(files):
            ...
            for line in f2.readlines():
                ...
                bb = [x1, y1, w, h]
                cls_name=convert_clsid(c)
                predictions.append({'image_id':int(file[:-4]), 'category_id':cls_name, 'bbox':[round(b, 3) for b in bb], 'score':round(float(s), 5)})
            f2.close()
